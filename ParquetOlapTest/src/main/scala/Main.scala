...
//      .config("spark.sql.catalog.local.warehouse", "s3a://ecommerce-warehouse/")
    ...
    println("Rozpoczynam benchmark analityczny OLAP dla surowego formatu Parquet...")

    val basePath = "s3a://ecommerce-warehouse/warehouse" // upewnij się, że masz tu swoje 4 tabele

    // Wczytanie i utworzenie widoków
    spark.read.parquet(s"$basePath/parquet_customers").createOrReplaceTempView("customers")
    spark.read.parquet(s"$basePath/parquet_products").createOrReplaceTempView("products")
    spark.read.parquet(s"$basePath/parquet_orders").createOrReplaceTempView("orders")
    spark.read.parquet(s"$basePath/parquet_order_items").createOrReplaceTempView("order_items")

    val queries = Map(
      "Q1 (Point Lookup na order_items)" ->
        "SELECT * FROM order_items WHERE order_id = 5000000",

      "Q2 (Agregacja z filtrem czasowym na orders)" ->
    ...
    for ((name, sql) <- queries) {
      ...
      for (i <- 1 to runs) {
        ...
        println(f"---> Średni czas nr $i ($name): ${(t1 - t0) / 1e9d}%.3f sekund\n")
      }
